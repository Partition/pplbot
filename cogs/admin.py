...
from config import GUILD_ID, MODERATOR_ROLE, TAG_CHARACTER_LIMIT
...
class Admin(commands.Cog):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        ...
        mod_role = discord.utils.get(interaction.guild.roles, id=MODERATOR_ROLE)
        if mod_role in member.roles:
            return True
        else:
            return await interaction.response.send_message(embed=EmbedGenerator.error_embed(title="Error", description="This is a moderator only command."), ephemeral=True)
    ...
    @commands.command()
    async def gsync(self, ctx, guild_id: int):
        """Sync the bot's commands to a specific guild.
        
        Parameters
        ----------
        guild_id: int
            The ID of the guild to sync commands to
        """
    ...
    async def create_channels(self, interaction: discord.Interaction, team_tag: str):
        """Create all team channels and category.
        
        Parameters
        ----------
        team_tag: str
            The tag of the team to create channels for
        """
        async with AsyncSessionLocal() as session:
    ...
    @app_commands.command()
    async def editleague(self, interaction: discord.Interaction, search_term: str, league: TeamLeague):
        """Edit the league of a team.
        
        Parameters
        ----------
        search_term: str
            The tag or name of the team to edit
        league: TeamLeague
            The league to set the team to
        """
        async with AsyncSessionLocal() as session:
    ...
    async def archive(self, interaction: discord.Interaction, search_term: str):
        """Archive a team and remove all associated channels/roles.
        
        Parameters
        ----------
        search_term: str
            The tag or name of the team to archive
        """
    ...
    async def stop(self, interaction: discord.Interaction):
        """Stop the bot and unload all cogs.
        """
